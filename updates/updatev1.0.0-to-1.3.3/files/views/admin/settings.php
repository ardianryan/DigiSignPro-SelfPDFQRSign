...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['action'])) {
    ...
    if (!$error_msg) {
        ...
        // Auto-migration for S3 columns
        $check_s3_pub = $conn->query("SHOW COLUMNS FROM app_settings LIKE 's3_public_url'");
        if ($check_s3_pub->num_rows == 0) {
            @$conn->query("ALTER TABLE app_settings ADD COLUMN s3_public_url VARCHAR(255) DEFAULT NULL");
        }
        $check_s3_dir = $conn->query("SHOW COLUMNS FROM app_settings LIKE 's3_directory'");
        if ($check_s3_dir->num_rows == 0) {
            @$conn->query("ALTER TABLE app_settings ADD COLUMN s3_directory VARCHAR(255) DEFAULT 'digisign/'");
        }
