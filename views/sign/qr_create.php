...
if ($step == 3 && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['pdf_file']) && $_FILES['pdf_file']['error'] == 0) {
        ...
        if ($ext != 'pdf') {
            ...
            // Get password from session (Mandatory from Step 1)
            $pdf_pass = $_SESSION['tte_qr_password'] ?? '';
            
            if (empty($pdf_pass)) {
                // Should not happen if Step 1 enforced it, but handle session expiry
                $error = "Sesi habis. Silakan ulangi proses.";
            } else {
                $uploadSuccess = false;
                ...
                $stmt = $conn->prepare("UPDATE signatures SET file_path = ?, document_name = ? WHERE id = ?");
                $stmt->bind_param("ssi", $db_path, $orig_name, $id);
                $stmt->execute();
