...
try {
    ...
    if (is_array($files)) {
        ...
        usort($files, function($a, $b) {
            $dateA = $a['LastModified'] ?? 0;
            $dateB = $b['LastModified'] ?? 0;
            return $dateB <=> $dateA;
        });
    } else {
        $files = [];
    ...
    $stats = null;
    $files = [];
    $msg = "Gagal memuat data storage: " . $e->getMessage();
    $msg_type = 'error';
}
...
<?php if ($msg): ?>
    <div class="mb-6 p-4 rounded-lg <?php echo $msg_type === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'; ?> flex items-center gap-3">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                                    <?php 
                                        ...
                                        if ($lm instanceof DateTimeInterface) {
                                            echo $lm->format('d M Y H:i:s');
                                        } else {
                                            echo date('d M Y H:i:s', is_numeric($lm) ? $lm : strtotime($lm));
                                        }
                                    ?>
                                </td>
